...
import Simple_Input from "../pages/component/Simple_Input";
import { MdEvent } from "react-icons/md";
import loading from "../pages/asset/qr-code-animation.gif";
import Simple_Select from "../pages/component/Simple_Select"
import "../pages/component/Online_Registration_Form.css"
import Input_Helper from "../pages/Helper/Input_Helper ";

import Swal from "sweetalert2";
import LocalHelper from "../pages/Helper/LocalHelper";

const Registration_SendBox = () => {
    //************************************/
    //Declration
    ...
    function LoadingData() {
      axios
        .get('https://us-central1-msa-server-db.cloudfunctions.net' + "/api/sandbox/generate/qr-code/url/" + keyURLRegistration)
        .then((data) => {
          if (data.data) {

            console.log(data.data.url_date.type)
            if (data.data.url_date.type == "pr") {
                set_data_Type_of_URL("pr");
            }else{
                set_data_Type_of_URL("qrcode");
            }

            set_data_URL_Registration(data.data.url_date)
            set_data_URL_Registration_Form(data.data.url_date.form);
            set_value_HeaderTitle(data.data.url_date.url_name);
            set_value_Footer_Note(
                data.data.url_date.on_register_form.footer_note
                    );
                    set_value_Footer_Contact(
                        data.data.url_date.on_register_form.contact_us
                    );

                    set_is_Loading(false)
                  //  console.log(data.data.list_event)

                    var listEvent = [];
                        for (let i of data.data.list_event) {
    ...
    function PrepareWriteToServer(listOfApprovedInput) {
      ...
      if (listEvent.length > 0) {
        ...
        axios
          .post('https://us-central1-msa-server-db.cloudfunctions.net' + "/api/sandbox/guest/resgisterd-form/create", data)
          .then((data) => {
            var sendData = {
            ...
            Swal.fire({
              ...
              window.location.replace(
                "/sandbox/v1/completed/registration-event/?data=" +
        ...
        axios
          ...
          .then((dataResponse) => {
            ...
            listEventFullData.map((rowEventFull, k) => {
              ...
              axios
                .post(host_name + "/api/sandbox/guest/resgisterd-form/create", data)
                .then((response) => {
...
export default Registration_SendBox
