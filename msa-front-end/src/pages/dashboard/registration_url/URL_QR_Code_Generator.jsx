...
const Qr_Code_Generator = () => {
  ...
  function Loading_URL_Detail() {
    axios
      .get(
        host_name_Server + api_registration_url + "/" + key_of_url_registation,
        {
          headers: {
            Authorization: token,
          },
      ...
      .then((data) => {
        ...
        if (data.data.message != "No Data!") {
          var listEvent = [];
          set_value_data_Of_URL_Registration(data.data.data);
          if (data.data.data.event) {
            data.data.data.event.map((row, index) => {
              axios
                .get(host_name_Server + api_event + "/" + row.event_key, {
                  headers: {
                    Authorization: token,
                  },
                })
                .then((dataEvent) => {
                  listEvent.push(dataEvent.data.data);

                  if (index + 1 == data.data.data.event.length) {
                    set_value_List_Of_Event(listEvent);
                    qrCodeShow(data.data.data, listEvent);
                    set_is_Loading(false);
                  }
                });
            });
          } else {
            set_value_List_Of_Event([]);
            qrCodeShow(data.data.data, []);
            set_is_Loading(false);
          }
        } else {
          alert("Invalid Param Key URL");
        }
      });
  }
  ...
  function qrCodeShow(urlCreate, event) {
    ...
    var qr =
      host_name_Server + "/event/registration/?key=" + encrypt(urlCreate.key);
    set_value_show_in_Qr_value_show_in_Qr_Fronted(
      host_name_Website + "/event/registration/?key=" + encrypt(urlCreate.key)
    );
